            ...
            <div class="p-4 border-t border-gray-100 dark:border-gray-800 sm:p-6 bg-orange-200 rounded-2xl">
                <div class="space-y-5">
                    <div class="overflow-hidden rounded-xl border border-gray-200 bg-gray-50 dark:border-gray-800">
                                        ...
                                        <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-200">
                                            ...
                                            <td class="px-5 py-4 sm:px-6 flex flex-row gap-2">
                                                ...
                                                <button wire:navigate
                                                    href="{{ route('admin.editbudget', ['id' => $budget->id]) }}"
                        ...
                        {{-- modal dialog --}}
                        {{-- modal dialog --}}
                        <div x-show="showModal" style="display: none;" class="relative z-10"
                            aria-labelledby="modal-title" role="dialog" aria-modal="true">
                            {{-- modal background --}}
                            <div x-show="showModal" x-transition:enter="ease-out duration-300"
                                x-transition:enter-start="opacity-25" x-transition:enter-end="opacity-25"
                                x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-25"
                                x-transition:leave-end="opacity-25"
                                class="fixed inset-0 bg-gray-500 bg-opacity-25 opacity-25 transition-opacity"></div>
                                ...
                                <div
                                    ...
                                    <div x-show="showModal" x-transition:enter="ease-out duration-300"
                                            ...
                                            <div class="sm:flex sm:items-start">
                                                <div
                                                    ...
                                                    <svg class="h-6 w-6 text-red-600" fill="none" viewBox="0 0 24 24"
                                                        ...
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898
                                             0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                                                    </svg>
                                                </div>
                                                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                                    <h3 class="text-base font-semibold leading-6 text-gray-900"
                                                        id="modal-title">Hapus Data Budget</h3>
                                                    <div class="mt-2">
                                                        <p class="text-sm text-gray-500">Yakin mau hapus data budget
                                                            ini?</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="bg-amber-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                            {{-- tombol modal konformasi --}}
                                            <button type="button"
                                                @click="$wire.deletebudget(budgetId); showModal = false"
                                                class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto">
                                                Ya, Hapus
                                            </button>
                                            {{-- tombol batal --}}
                                            <button type="button" @click="showModal = false"
                                                class="mt-3 inline-flex w-full justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white@ shadow-sm ring-1 ring-inset ring-gray-300
                                     hover:bg-indigo-500 sm:mt-0 sm:w-auto">
                                                Batal
                                            </button>
                                        </div>
                                    </div>
