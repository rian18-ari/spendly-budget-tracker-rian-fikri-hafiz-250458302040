    ...
    <div class="flex flex-col md:flex-row gap-4">
                ...
                @auth
                    @if (!$user->image)
                        <img src="{{ asset('asset/img/profil default instagram.jpeg') }}" class="w-full h-full rounded-full"
                            alt="">
                    @else
                        <img src="{{ asset('storage/' . $user->image) }}" class="w-full h-full rounded-full" alt="">
                    @endif
                @endauth
            </div>
        ...
        <div class="flex flex-col gap-2 p-2 w-full h-full rounded-lg mt-2">
            ...
            <div class="bg-gray-300 p-2 rounded-lg gap-2 h-full">
                <p class="text-lg font-semibold">Nama: <span class="font-normal">{{ $user->name }}</span></p>
                <hr class="border-gray-500 w-full md:w-1/2 items-center my-2">
                <p class="text-lg font-semibold">Email: <span class="font-normal">{{ $user->email }}</span></p>
                <hr class="border-gray-500 w-full md:w-1/2 items-center my-2">
                <p class="text-lg font-semibold">Role: <span class="font-normal">{{ $user->role }}</span></p>
                <hr class="border-gray-500 w-full md:w-1/2 items-center my-2">
                <p class="text-lg font-semibold">Dibuat: <span
                        class="font-normal">{{ $user->created_at->format('d-m-Y') }}</span></p>
                <hr class="border-gray-500 w-full md:w-1/2 items-center my-2">
                <p class="text-lg font-semibold">Diupdate: <span
                        class="font-normal">{{ $user->updated_at->format('d-m-Y') }}</span></p>
            </div>
            <div class="flex justify-end">
                <a wire:navigate href="{{ route('dashboard') }}"
                    class="w-full md:w-1/4 text-center px-6 py-2 bg-red-600 text-white font-semibold rounded-lg shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition duration-150 ease-in-out">Kembali</a>
            </div>
        </div>
    </div>
</div>
